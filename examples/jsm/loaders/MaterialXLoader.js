...
import { Loader, FileLoader, TextureLoader, RepeatWrapping } from '../../../build/three.module.js';
import { abs, sign, floor, ceil, round, sin, cos, tan, asin, acos, sqrt, exp, clamp, min, max, normalize, length, dot, cross, normalMap, remap, smoothstep, luminance, mx_rgbtohsv, mx_hsvtorgb, mix, vec2, vec3, vec4, mx_ramplr, mx_ramptb, mx_splitlr, mx_splittb, mx_noise_float, mx_fractal_noise_float, mx_cell_noise_float, mx_worley_noise_float, mx_safepower, mx_contrast, saturation, add, float, sub, mul, div, mod, pow, atan2, positionWorld, positionLocal, normalWorld, normalLocal, tangentWorld, tangentLocal, uv, vertexColor, mx_transform_uv, texture, color, MeshBasicNodeMaterial, MeshPhysicalNodeMaterial, mx_srgb_texture_to_lin_rec709, int, bool } from '../nodes/Nodes.js';
...
class MaterialXNode {
	...
	setMaterial( material ) {
		...
		if ( element === 'gltf_pbr' ) ; else if ( element === 'standard_surface' ) {
